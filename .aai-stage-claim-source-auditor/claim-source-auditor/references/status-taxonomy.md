...
**Package note:** 2026-09-03 remediation did not add a partial status. The AAI profile's partial support maps to `not found in searched sources` or `manual review needed`.
...
The auditor cannot confirm or deny the claim after all safe available extraction routes have been exhausted. Use when the source is likely to contain the supporting content but OCR, alternate extraction, rendered-page inspection, a dedicated file-format workflow, and other available safe routes still cannot establish it, or when the remaining issue genuinely requires human judgment.

**Required basis:** Every safe extraction route attempted and why each remained insufficient.
**Example basis:** "Claim asserts a verbal referral. Direct extraction and OCR failed on the handwritten note at page 47; rendered-page inspection remains ambiguous. Manual review required."
...
6. Use **manual review needed** only after all safe available extraction routes fail on an otherwise relevant source, or the remaining ambiguity genuinely requires human judgment. Record the attempted routes.
7. When uncertain between **verified** and another status after exhausting safe routes, do not award verified. Use the status supported by the record, including **manual review needed** for genuine unresolved ambiguity. The downstream cost of a false positive exceeds the cost of a false negative.
...
| claim_id | ✓ | string |
| claim_text | ✓ | string |
| status | ✓ | one of the five taxonomy values |
...
| source_excerpt | conditional | string (≤300 chars; required when status is verified, verified in broader bundle, or conflicting) |
| basis_note | ✓ | string (≤200 chars; explains the status assignment) |
